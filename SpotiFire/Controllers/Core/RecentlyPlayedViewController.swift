...
class RecentlyPlayedViewController: UIViewController, UISearchResultsUpdating, UISearchBarDelegate {
    
    let searchController: UISearchController = {
        let vc = UISearchController(searchResultsController: RecentlyPlayedResultsViewController())
    ...
    private var collectionView: UICollectionView = UICollectionView(
        ...
        return RecentlyPlayedViewController.createSectionLayout(section: sectionIndex)
        }
    )
    ...
    private var newUserRecentlyPlayed: [RecentTrackResponse] = []
    ...
    override func viewDidLoad() {
        ...
        RecentlyPlayedViewController.selectedItems = [:]
        fetchTracks()
    }
    ...
    private func fetchTracks() {
        ...
        group.enter()
        var userRecentlyPlayed: RecentlyPlayedResponse?
        APICaller.shared.getUserRecentlyPlayed(limit: 50) { result in
            defer {
                group.leave()
            }
            switch result {
            case.success(let model):
                userRecentlyPlayed = model
        ...
        group.notify(queue: .main) {
            self.newUserRecentlyPlayed = userRecentlyPlayed?.items ?? []
            self.configureModel(recentlyPlayed: self.newUserRecentlyPlayed)
            if self.newUserRecentlyPlayed.count < 1 {
                HapticsManager.shared.vibrate(for: .warning)
                let alert = UIAlertController(
                    title: "Alert",
    ...
    private func configureModel(
        recentlyPlayed: [RecentTrackResponse]
    ) {
        recentlyPlayedSection.append(contentsOf: recentlyPlayed.compactMap({
            return TrackCellViewModel(
                image: URL(string: $0.track.album.images.first?.url ?? ""),
                track_name: $0.track.name,
                artist_name: $0.track.artists.first?.name ?? "-",
                album_name: $0.track.album.name,
                id: $0.track.id
            )
        }))
...
//        }
    ...
    func updateSearchResults(for searchController: UISearchController) {
        guard let resultsController = searchController.searchResultsController as? RecentlyPlayedResultsViewController,
    ...
    @objc func didTapEmpty() {
        if RecentlyPlayedViewController.selectedItems.count < 1 {
            HapticsManager.shared.vibrate(for: .warning)
            let alert = UIAlertController(
                title: "Alert",
    ...
    @objc private func showSummary() {
        if RecentlyPlayedViewController.selectedItems.count < 1 {
            ...
            let str = "\n\n\n"
            let repeatStr = String(repeating: str, count: RecentlyPlayedViewController.selectedItems.count)
            let alert = UIAlertController(
            ...
            var i = 0
            for (key, value) in RecentlyPlayedViewController.selectedItems {
                ...
                label.onClick = {
                    ...
                    removeAlert.addAction(UIAlertAction(
                        ...
                        style: .default,
                        handler: {_ in
                            RecentlyPlayedViewController.selectedItems.removeValue(forKey: key)
                            if self.recentlyPlayedSection.first(where: { $0.id == key }) != nil {
            ...
            alert.addAction(UIAlertAction(
                ...
                handler: { (result : UIAlertAction) -> Void in
                    let vc = RecommendationsViewController(
                        selectedArtistsID: [],
                        selectedTracksID: RecentlyPlayedViewController.selectedItems.map{String($0.key)},
                        selectedGenresID: [],
...
extension RecentlyPlayedViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didDeselectItemAt indexPath: IndexPath) {
        ...
        if RecentlyPlayedViewController.selectedItems.isEmpty {
            navigationItem.rightBarButtonItems = [infoIcon]
        }
    }
}
