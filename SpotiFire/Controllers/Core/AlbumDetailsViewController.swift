...
class AlbumDetailsViewController: UIViewController {
    ...
    private let album: AlbumsResponse
    init(album: AlbumsResponse) {
        self.album = album
    ...
    private var newAlbumTracks: [AlbumTrack] = []
    ...
    override func viewDidLoad() {
        ...
        AlbumDetailsViewController.selectedItems = [:]
    ...
    private func fetchAlbumDetails() {
        ...
        group.notify(queue: .main) {
            APICaller.shared.getAlbumTracks(album: self.album, offset: self.newAlbumTracks.count) { [weak self] result in
                DispatchQueue.main.async {
                    ...
                    case .success(let model):
                        ...
                        self?.newAlbumTracks.append(contentsOf: model.items)
                        if self?.newAlbumTracks.count ?? 0 < 1 {
                            HapticsManager.shared.vibrate(for: .warning)
                            let alert = UIAlertController(
                                title: "Alert",
                                message: "No album track found",
                        ...
                        for (_, value) in AlbumDetailsViewController.selectedItems {
    ...
    @objc func didTapEmpty() {
        if AlbumDetailsViewController.selectedItems.count < 1 {
            HapticsManager.shared.vibrate(for: .warning)
            let alert = UIAlertController(
                title: "Alert",
    ...
    @objc private func showSummary() {
        if AlbumDetailsViewController.selectedItems.count < 1 {
            ...
            let str = "\n\n\n"
            let repeatStr = String(repeating: str, count: AlbumDetailsViewController.selectedItems.count)
            let alert = UIAlertController(
            ...
            var i = 0
            for (key, value) in AlbumDetailsViewController.selectedItems {
                ...
                label.onClick = {
                    ...
                    removeAlert.addAction(UIAlertAction(
                        ...
                        style: .default,
                        handler: {_ in
                            AlbumDetailsViewController.selectedItems.removeValue(forKey: key)
            ...
            alert.addAction(UIAlertAction(
                ...
                handler: { (result : UIAlertAction) -> Void in
                    let vc = RecommendationsViewController(
                        selectedArtistsID: [],
                        selectedTracksID: AlbumDetailsViewController.selectedItems.map{String($0.key)},
                        selectedGenresID: [],
...
extension AlbumDetailsViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        let headerViewModel = AlbumHeaderViewViewModel(
            ...
            artworkURL: URL(string: album.album.images.first?.url ?? "")
        )
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if AlbumDetailsViewController.selectedItems.count > 4 {
            HapticsManager.shared.vibrate(for: .warning)
            ...
            AlbumDetailsViewController.selectedItems[viewModels[indexPath.row].id] = (viewModels[indexPath.row].track_name, indexPath)
        }
    }

    func collectionView(_ collectionView: UICollectionView, didDeselectItemAt indexPath: IndexPath) {
        HapticsManager.shared.vibrateForSelection()
        if AlbumDetailsViewController.selectedItems.keys.contains(viewModels[indexPath.row].id) {
            AlbumDetailsViewController.selectedItems.removeValue(forKey: viewModels[indexPath.row].id)
        }
        if AlbumDetailsViewController.selectedItems.isEmpty {
            navigationItem.rightBarButtonItems = [infoIcon]
        }
    }
